...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      {/* ══ CONTACT STRIP ══ */}
      {/* <section style={{ background: "linear-gradient(135deg, #1a1a2e, #16213e)", padding: "4rem" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "2.5rem", alignItems: "center" }}>
          ...
          {[
            ...
            { icon: Mail, label: "Email", val: "info@taasaupvc.com" },
            { icon: MapPin, label: "Address", val: "Ahmedabad, Gujarat, India" },
          ].map(({ icon: Icon, label, val }) => (
      ...
      {/* ══ FOOTER ══ */}
      {/* <footer style={{ background: "#12121f", padding: "1.8rem 4rem", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
          <p style={{ fontSize: "0.7rem", color: "#3a3a4a", letterSpacing: "0.07em" }}>© 2026 TAASA Industries. All rights reserved. Designed for Performance.</p>
          <div style={{ display: "flex", gap: "1.8rem" }}>
            {["Privacy Policy", "Terms", "Sitemap"].map(l => (
              <span key={l} style={{ fontSize: "0.65rem", color: "#3a3a4a", cursor: "pointer", letterSpacing: "0.07em", transition: "color 0.3s" }}
                onMouseEnter={e => e.target.style.color = "#c49138"} onMouseLeave={e => e.target.style.color = "#3a3a4a"}>{l}</span>
            ))}
          </div>
        </div>
      </footer> */}
